...
//These contol the speed at which the bus comms.  4/12 = 3 and 1/3 MHz bitclock.
#define WS_I2S_BCK 4  //Can't be less than 1.
#define WS_I2S_DIV 12
...
//printf( "%08x\n", slc_intr_status );
	if ( (slc_intr_status & SLC_RX_EOF_INT_ST))
	{
		...
		if( ledreadyflag && finishedDesc == &i2sBufDescTX[2] )
		{
			i2sBufDescTX[1].next_link_ptr=(int)&i2sBufDescTX[0];
			ledreadyflag = 0;
		}
		if( finishedDesc == &i2sBufDescTX[1] )
		{
	...
	if ( (slc_intr_status & SLC_TX_EOF_INT_ST))
	{
		...
		uint32_t * rxbuf = (uint32_t*)finishedDesc->buf_ptr;
		vive_data( rxbuf, I2SDMABUFLEN );
/*		int i;
		vlast = rxbuf[0];
...
void ICACHE_FLASH_ATTR testi2s_init() {
	...
	//Tricky make a graph, with node 0 only callable once.
	i2sBufDescTX[0].next_link_ptr=(int)&i2sBufDescTX[1];
	i2sBufDescTX[1].next_link_ptr=(int)&i2sBufDescTX[2];
	i2sBufDescTX[2].next_link_ptr=(int)&i2sBufDescTX[1];
...
void ICACHE_FLASH_ATTR ws2812_push( uint8_t * buffer,  int ledcount)
{
	...
	uint16_t * bufferpl = (uint16_t*)&i2sBDTX[32];
	int buffersize = ledcount * 3;
	int place;
